...
class ChannelProvider(private val iName: String, private val dName: String) {
    ...
    private fun create(): Long {
        ...
        val channelUri = App.appContext().contentResolver.insert(
            TvContractCompat.Channels.CONTENT_URI,
            builder.build().toContentValues()
        )
        channelId = channelUri?.let { ContentUris.parseId(it) } ?: 0
        val bitmap = BitmapFactory.decodeResource(App.appContext().resources, R.drawable.ts_round)
        ChannelLogoUtils.storeChannelLogo(App.appContext(), channelId, bitmap)
        TvContractCompat.requestChannelBrowsable(App.appContext(), channelId)
        return channelId
    }

    fun update(list: List<Torrent>) {
        val channelId = create()
        App.appContext().contentResolver.delete(
        ...
        App.appContext().contentResolver.update(
        ...
        if (list.isNotEmpty())
            list.forEachIndexed { index, torrent ->
                ...
                App.appContext().contentResolver.insert(
    ...
    private fun emptyProgram(channelId: Long): PreviewProgram {
        val vintent = Intent(App.appContext(), MainActivity::class.java)
        vintent.flags = Intent.FLAG_ACTIVITY_NEW_TASK
        val resourceId = R.drawable.emptyposter
        val ep = Uri.Builder()
            .scheme(ContentResolver.SCHEME_ANDROID_RESOURCE)
            .authority(App.appContext().resources.getResourcePackageName(resourceId))
            .appendPath(App.appContext().resources.getResourceTypeName(resourceId))
            .appendPath(App.appContext().resources.getResourceEntryName(resourceId))
            .build()
        val preview = PreviewProgram.Builder()
            .setChannelId(channelId)
            .setTitle(App.appContext().getString(R.string.app_name))
            .setAvailability(TvContractCompat.PreviewProgramColumns.AVAILABILITY_FREE)
            .setDescription(App.appContext().getString(R.string.open_torrserve))
            .setReviewRating("5")
            .setIntent(vintent)
    ...
    private fun rem(ch: Channel) {
        App.appContext().contentResolver.delete(TvContractCompat.buildChannelUri(ch.id), null, null)
    }
}
